...
tux@suse16:~ > sudo zypper install cmake-full
```
...
1.  **下載與解壓縮**:
    ...
    ```bash
    tux@suse16:~ > tar zxf gromacs-2025.3.tar.gz
    tux@suse16:~ > cd gromacs-2025.3
    ```
...
2.  **建立獨立的建構目錄**:
    ...
    ```bash
    # build
    tux@suse16:~/gromacs-2025.3 > mkdir build
    tux@suse16:~/gromacs-2025.3 > cd build
    ```
...
3.  **使用 CMake 設定建構選項**:
    ...
    ```bash
    tux@suse16:~/gromacs-2025.3/build > cmake -DGMX_FFT_LIBRARY=mkl -DGMX_SIMD=AVX2_256 -DGMX_MPI=ON -DGMX_OPENMP=ON -DBUILD_SHARED_LIBS=ON -DGMX_DOUBLE=OFF -DGMX_GPU=OFF -DGMX_HWLOC=OFF -DCMAKE_C_COMPILER=mpiicx -DCMAKE_CXX_COMPILER=mpiicpx -DCMAKE_BUILD_TYPE=Release -DCMAKE_COLOR_MAKEFILE=ON -DCMAKE_VERBOSE_MAKEFILE=TRUE -DCMAKE_INSTALL_PREFIX=$HOME/gromacs ..
    ```
...
4.  **編譯、測試與安裝**:
    ...
    ```bash
    tux@suse16:~/gromacs-2025.3/build > make
    tux@suse16:~/gromacs-2025.3/build > make check
    tux@suse16:~/gromacs-2025.3/build > make install
    ```
...
```bash
tux@suse16:~ > source $HOME/gromacs/bin/GMXRC
```
